...
export const handler = async (event) => {
  try {
    ...
    if (!uid) return { statusCode: 400, body: JSON.stringify({ error: "Missing uid parameter" }) };
    ...
    if (!sheetIds.length) return { statusCode: 200, body: JSON.stringify({ spreadsheets: [] }) };
    ...
    try {
      ...
      console.error("Failed to parse Sheet credentials:", err);
      throw err;
    }
