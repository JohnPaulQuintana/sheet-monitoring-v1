...
const TELEGRAM_CHAT_ID = "8260591122";
...
exports.handler = async function () {
  try {
    // 👥 Loop through all users (or a specific list)
    ...
    for (const doc of usersSnapshot.docs) {
      ...
      // Filter not updated sheets
      const notUpdatedSheets = [];
      sheetsData.forEach((sheet) => {
        sheet.sheets.forEach((s) => {
          if (s.status === "not_updated") {
            notUpdatedSheets.push(`- ${sheet.title} / ${s.title}`);
          }
        });
      });

      if (notUpdatedSheets.length) {
        const message = `⚠️ *Sheets Not Updated Today* for user ${uid}:\n` + notUpdatedSheets.join("\n");
