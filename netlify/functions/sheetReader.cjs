...
exports.handler = async function (event) {
  ...
  try {
    ...
    // ✅ Step 2: Fetch all sheet data in parallel (instead of sequentially)
    const results = await Promise.allSettled(
      sheetIds.map(async (spreadsheetId) => {
        try {
          ...
          console.error(`Error reading sheet ${spreadsheetId}:`, err.message);
          return null;
        }
      })
    );
    ...
    console.error("SheetReader Error:", error);
