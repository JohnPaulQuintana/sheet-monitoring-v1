...
async function runInBatches(ids, batchSize = 5) {
  const allResults = [];
  for (let i = 0; i < ids.length; i += batchSize) {
    const batch = ids.slice(i, i + batchSize);
    const results = await Promise.allSettled(batch.map((id) => processSheet(id)));
    for (const r of results) if (r.status === "fulfilled") allResults.push(r.value);
  }
  return allResults;
}
...
exports.handler = async function (event) {
  ...
  try {
    ...
    // dynamic batch size
    let batchSize = 5;
    if (sheetIds.length > 50) batchSize = 3;
    else if (sheetIds.length > 20) batchSize = 4;

    console.log(`⚙️ Processing ${sheetIds.length} sheets (batch size ${batchSize})`);

    const allSheetsData = await runInBatches(sheetIds, batchSize);
