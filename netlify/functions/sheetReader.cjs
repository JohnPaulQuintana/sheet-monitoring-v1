...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (!sheetId) return { statusCode: 400, body: JSON.stringify({ error: "Missing sheet_id" }) };

    const sheetData = await processSheet(sheetId);

    console.timeEnd("sheetReader");
    return { statusCode: 200, body: JSON.stringify({ spreadsheet: sheetData }) };
  } catch (err) {
